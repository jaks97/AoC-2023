defmodule Day7 do
  ...
  defmodule Hand do
    @cards_ordered_by_strength [?J, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9, ?T, ?Q, ?K, ?A]
    ...
    def get_hand_type_score hand do
      case hand |> Enum.find_index(& &1 == ?J) do
        nil ->
          [
            &is_five_of_a_kind?/1,
            &is_four_of_a_kind?/1,
        ...
        card_position ->
          @cards_ordered_by_strength
            |> Enum.filter(& &1 != ?J)
            |> Enum.map(fn new_card ->
              hand |> List.replace_at(card_position, new_card) |> get_hand_type_score
            end)
            |> Enum.max
      end
    end

    def get_relative_strength(card), do: @cards_ordered_by_strength |> Enum.find_index(& &1 == card)
